...
// called from [pid].js getStaticPaths() function 
...
// called from [pid].js getStaticProps function
export async function getPData(idRequested) {

  // get filepath to json file
  const filePath = path.join(dataDir, 'persons.json');
  const filePath2 = path.join(dataDir, 'prelations.json');
...
// find matching owner_pid in prelations data model
  const objMatch2 = jsonObj2.filter(obj => {
    return obj.owner_pid.toString() === idRequested;
  }
  ...
  if (objMatch2.length > 0) {
    ...
    const objMatch3 = jsonObj.filter(obj => {
      return objMatch2[0].related_pids.includes( obj.pid );
    }
    );
